...
class BetaOscillations_Dataset(Dataset):
    ...
    def open_dataset(self, folder_beta, folder_timefreq, folder_stim, mode, task):
        ...
        if task == 'day':
          ...
          for betaoscillations_segment in BetaOscillations_AllFiles_DAY:
          ...
          random.shuffle(BetaOscillations_DAY_DATASET)
          print(len(BetaOscillations_DAY_DATASET))
          # Calculate the split indices
          split_1 = int(len(BetaOscillations_DAY_DATASET) * 0.85)
          split_2 = int(len(BetaOscillations_DAY_DATASET) * 1)
          # Split the list
          if mode == 'train':
            data_list = BetaOscillations_DAY_DATASET[:split_1]
          elif mode == 'val':
            data_list = BetaOscillations_DAY_DATASET[split_1:split_2]
          elif mode == 'test':
            data_list = BetaOscillations_DAY_DATASET[split_1:split_2] 
          ...
          print(f"{self.days} days -> 1 day after ...")
          for betaoscillations_segment in BetaOscillations_AllFiles_MORNING:
                if len(betaoscillations_segment['distribution']) > (self.days + self.days_after-1):
                    for i in range(len(betaoscillations_segment['distribution'])-(self.days + self.days_after-1)):
                        if 'Ch1' in betaoscillations_segment['driver_channel']:
                            single_recording = {'beta_distribution': np.array([betaoscillations_segment['distribution'][j]['HISTOnorm_beta_power_Ch1'] for j in range(i, i+self.days)]),
          ...
          # Shuffle the original list
          random.shuffle(BetaOscillations_MORNING_DATASET)
          print(len(BetaOscillations_MORNING_DATASET))
          # Calculate the split indices
          split_1 = int(len(BetaOscillations_MORNING_DATASET) * 0.85)
          split_2 = int(len(BetaOscillations_MORNING_DATASET) * 1)
          # Split the list
          if mode == 'train':
            data_list = BetaOscillations_MORNING_DATASET[:split_1]
          elif mode == 'val':
            data_list = BetaOscillations_MORNING_DATASET[split_1:split_2]
          elif mode == 'test':
            data_list = BetaOscillations_MORNING_DATASET[split_1:split_2]
          ...
          print(f"{self.days} days -> 1 day after ...")
          for betaoscillations_segment in BetaOscillations_AllFiles_NIGHT:
                if len(betaoscillations_segment['distribution']) > (self.days + self.days_after-1):
                    for i in range(len(betaoscillations_segment['distribution'])-(self.days + self.days_after-1)):
                        if 'Ch1' in betaoscillations_segment['driver_channel']:
                            single_recording = {'beta_distribution': np.array([betaoscillations_segment['distribution'][j]['HISTOnorm_beta_power_Ch1'] for j in range(i, i+self.days)]),
          ...
          # Shuffle the original list
          random.shuffle(BetaOscillations_NIGHT_DATASET)
          print(len(BetaOscillations_NIGHT_DATASET))
          # Calculate the split indices
          split_1 = int(len(BetaOscillations_NIGHT_DATASET) * 0.85)
          split_2 = int(len(BetaOscillations_NIGHT_DATASET) * 1)
          # Split the list
          if mode == 'train':
            data_list = BetaOscillations_NIGHT_DATASET[:split_1]
          elif mode == 'val':
            data_list = BetaOscillations_NIGHT_DATASET[split_1:split_2]
          elif mode == 'test':
            data_list = BetaOscillations_NIGHT_DATASET[split_1:split_2]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
          ...
          return beta_distribution_data, beta_distribution_dayAfter, patient_id, segment_id
